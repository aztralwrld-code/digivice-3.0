...
export const INITIAL_CREATURE: Creature = {
  ...
  kernel: {
    ...
    axes: {
        ...
        curiosity: 80,  
        sync: 10,
        stability: 50 // Baseline stability
    },
    memories: [],
    traumas: []
  },
...
export const EVOLUTION_TREE: Record<string, EvolutionNode> = {
  ...
  'agumon': {
    ...
    requirements: { 
        minStats: { str: 15 },
        kernelConditions: { minTrust: 30, maxStress: 40 } // Needs a stable home
    },
    next: ['greymon', 'darkgreymon'] // Branching
  },
  'betamon': {
    id: 'betamon',
    stage: DigimonStage.ROOKIE,
    requirements: { 
        minStats: { str: 10 },
        kernelConditions: { minTrust: 10 } // Easier, lower standards
    }, 
    next: ['seadramon']
  },
  'greymon': {
    ...
    requirements: { 
        minStats: { str: 50 },
        kernelConditions: { minTrust: 60, minSync: 40 } // Requires emotional bond
    },
    next: ['metalgreymon']
  },
  'darkgreymon': {
      ...
      requirements: {
          minStats: { str: 60 },
          kernelConditions: { maxStress: 100, requiresTrauma: true } // Stress must be HIGH
      },
      next: []
  },
  'seadramon': {
    ...
    requirements: { 
        minStats: { str: 40 },
        kernelConditions: { maxStress: 60 } // Can evolve under pressure
    },
    next: []
  },
  'metalgreymon': {
...
export const MENU_GRID = [
    { id: 'PARTNER', label: 'DATABASE', icon: 'USER' },
    { id: 'TRAIN', label: 'TRAINING', icon: 'DUMBBELL' },
    { id: 'FEED', label: 'NUTRITION', icon: 'UTENSILS' }, 
    { id: 'SCAN', label: 'NET SCAN', icon: 'WIFI' },
    { id: 'SHOP', label: 'MARKET', icon: 'SHOPPING_BAG' },
    { id: 'SLEEP', label: 'HIBERNATE', icon: 'MOON' }
];
...
export const TICK_RATE_MS = 3000; // Game loop speed
