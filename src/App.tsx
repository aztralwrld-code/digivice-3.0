...
function App() {
  ...
  useEffect(() => {
    if (gameState.currentScreen === ScreenMode.BOOT) {
        const timings = [1500, 3500, 5500, 8000];
        const t1 = setTimeout(() => setGameState(p => ({ ...p, bootPhase: BootPhase.KERNEL_LOAD })), timings[0]);
        const t2 = setTimeout(() => setGameState(p => ({ ...p, bootPhase: BootPhase.BIOMETRIC_HANDSHAKE })), timings[1]);
        const t3 = setTimeout(() => setGameState(p => ({ ...p, bootPhase: BootPhase.SYSTEM_READY })), timings[2]);
        const t4 = setTimeout(() => setGameState(p => ({ ...p, currentScreen: ScreenMode.LOGIN })), timings[3]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setGameState((prev: GameState) => {
        if (prev.currentScreen === ScreenMode.BOOT || prev.currentScreen === ScreenMode.LOGIN) return prev;
